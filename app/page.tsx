...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    {view === 'result' && <section className="mx-auto min-h-[calc(100vh-74px)] max-w-4xl px-5 py-12"><div className="w-full rounded-3xl border bg-card p-6 shadow-[0_25px_80px_rgba(15,35,42,.09)] sm:p-10"><div className="grid items-center gap-8 sm:grid-cols-[180px_1fr]"><div className="result-ring" style={{ '--score': `${scorePercent}%` } as React.CSSProperties}><div><strong>{correctCount}</strong><span>из {answeredQuestions.length}</span></div></div><div><Badge className={sessionPassed ? 'bg-[var(--mint)] text-[var(--mint-ink)]' : ''}>{mode === 'learn' ? 'Сессия завершена' : sessionPassed ? 'Экзамен пройден' : 'Порог не достигнут'}</Badge><h2 className="mt-4 text-3xl font-bold tracking-tight">{examYear}: {scorePercent}%</h2><p className="mt-3 leading-7 text-muted-foreground">{mode === 'learn' ? 'Вы сразу разобрали каждую ошибку. Повторите темы с низким результатом после короткого перерыва.' : level === 'IP' ? 'В MVP успешным считается результат от 60%. Официальная оценка IP дополнительно требует порога по каждой предметной области.' : `Для успешного FE нужно не менее 60% отдельно в каждой части: A — ${partScore('A')}%, B — ${partScore('B')}%.`}</p></div></div><div className="mt-9 grid gap-3 sm:grid-cols-3">{['Technology','Management','Strategy'].map((domain) => { const items = answeredQuestions.filter((item) => item.domain === domain || (domain === 'Technology' && ['Algorithm','Security'].includes(item.domain))); const correct = items.filter((item) => answers[item.id] === item.answer).length; return <div key={domain} className="rounded-2xl bg-muted/65 p-4"><p className="text-xs text-muted-foreground">{domain}</p><p className="mt-2 text-2xl font-bold">{correct} / {items.length}</p></div>; })}</div>
      <div className="mt-10 border-t pt-8"><div className="mb-5 flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-primary">Полный разбор</p><h3 className="mt-1 text-2xl font-bold tracking-tight">Что было отвечено и где ошибка</h3></div><Badge variant="outline">{correctCount} верно · {pool.length - correctCount} ошибок</Badge></div><div className="space-y-3">{pool.map((item, itemIndex) => { const answer = answers[item.id]; const isCorrect = answer === item.answer; return <details key={item.id} className={`review-item ${isCorrect ? 'review-correct' : 'review-wrong'}`}><summary><span className="review-number">{level === 'FE' ? `${item.part} · ` : ''}{itemIndex + 1}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{item.prompt}</span><span className="mt-1 block text-xs text-muted-foreground">{item.topic} · {answer === undefined ? 'нет ответа' : isCorrect ? 'верно' : 'ошибка'}</span></span>{isCorrect ? <CheckCircle2 className="size-5 shrink-0 text-[var(--success)]" /> : <XCircle className="size-5 shrink-0 text-destructive" />}</summary><div className="review-body"><div className="grid gap-3 sm:grid-cols-2"><div className={`rounded-xl p-3 ${isCorrect ? 'bg-[var(--mint)]' : 'bg-destructive/8'}`}><p className="text-xs font-semibold text-muted-foreground">Ваш ответ</p><p className="mt-1 text-sm font-medium">{answer === undefined ? 'Ответ не выбран' : item.options[answer]}</p></div><div className="rounded-xl bg-[var(--mint)] p-3"><p className="text-xs font-semibold text-muted-foreground">Правильный ответ</p><p className="mt-1 text-sm font-medium">{item.options[item.answer]}</p></div></div><p className="mt-4 text-sm leading-6 text-muted-foreground">{item.explanation}</p><div className="mt-4 space-y-2 border-t pt-4">{item.optionNotes.map((note, optionIndex) => <p key={optionIndex} className="text-sm leading-6"><strong className={optionIndex === item.answer ? 'text-[var(--success)]' : 'text-muted-foreground'}>{item.options[optionIndex]}:</strong> {note}</p>)}</div><div className="mt-4 flex gap-2 rounded-xl bg-background p-3 text-sm leading-6"><Sparkles className="mt-1 size-4 shrink-0 text-primary" /><span><strong>Запомнить:</strong> {item.takeaway}</span></div></div></details>; })}</div></div>
      <div className="mt-8 flex flex-wrap justify-end gap-3 border-t pt-6"><Button variant="outline" onClick={reset}><ArrowLeft data-icon="inline-start" />К настройке</Button><Button onClick={startQuiz}><RotateCcw data-icon="inline-start" />Пройти ещё раз</Button></div></div></section>}
  </main>;
}
