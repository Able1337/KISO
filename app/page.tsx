...
import { ArrowLeft, ArrowRight, BarChart3, BookOpenCheck, CalendarDays, Check, CheckCircle2, Clock3, GraduationCap, Lightbulb, Pause, Play, RotateCcw, Search, ShieldCheck, Sparkles, Target, Timer, Trophy, X, XCircle } from 'lucide-react';
...
import OfficialExams, { officialPack, type OfficialEntry } from './official-exams';
import { readStorage, summarize, STORAGE_KEY, type ExamStorage } from '@/lib/exam-session';
import { officialLessonCount } from './official-lessons';
...
export default function Home() {
  const [officialEntry,setOfficialEntry]=useState<OfficialEntry|null>(null);
  const [officialStorage,setOfficialStorage]=useState<ExamStorage>({version:1,active:null,history:[]});
  const [bankChoice,setBankChoice]=useState<'official'|'mvp'>('official');
  ...
  const questionText = question ? originalText[question.id][system] : null;
  ...
  const currentStudyRecommendation = question ? studyRecommendations[question.id][uiLanguage] : null;
  ...
  function refreshOfficial() {try {setOfficialStorage(readStorage(localStorage.getItem(STORAGE_KEY),officialPack));} catch { /* The exam screen reports unavailable storage. */ } setOfficialLoaded(true);}
  useEffect(()=>{refreshOfficial();const refresh=()=>refreshOfficial();window.addEventListener('storage',refresh);return()=>window.removeEventListener('storage',refresh);},[]);
  const officialAvailable=system==='ITPEC'&&level==='IP'&&examYear===2026;
  const useOfficial=officialAvailable&&bankChoice==='official';
  const label=(ru:string,en:string,ja:string)=>uiLanguage==='ru'?ru:uiLanguage==='en'?en:ja;
  const officialPaperLabel=label('Апрель 2026 · 100 вопросов','April 2026 · 100 questions','2026年4月 · 100問');
  const shownProgress=useOfficial?officialStorage.history.reduce((v,a)=>{const summary=summarize(officialPack,a);return {attempts:v.attempts+summary.answered,correct:v.correct+summary.correct};},{attempts:0,correct:0}):progress;
  ...
  useEffect(() => {
    ...
    const attempt: ExamAttempt = { id: `${completedAt}-${system}-${level}-${examYear}`, system, level, year: examYear, mode, correct: correctCount, total: pool.length, passed: sessionPassed, completedAt, ...(level === 'FE' ? { partScores: { A: partScore('A'), B: partScore('B') } } : {}) };
  ...
  useEffect(() => {
    ...
    void Promise.resolve(context.registerTool({
      ...
      execute(input) {
        ...
        if(value.system==='ITPEC'&&value.level==='IP'&&value.year===2026) {
          setSystem('ITPEC');setLevel('IP');setExamYear(2026);setMode(value.mode!);setBankChoice('official');
          setOfficialEntry({mode:value.mode!});
          return {status:'started',system:'ITPEC',level:'IP',year:2026,mode:value.mode,questions:100};
        }
        ...
        setIndex(0); setAnswers({}); setSelected(null); setBreakSeconds(10 * 60); setSessionRecorded(false);
  ...
  function startQuiz() { if(useOfficial){setOfficialEntry({mode});return;} const initialPart: Part = level === 'FE' ? 'A' : 'single'; setPart(initialPart); setIndex(0); setAnswers({}); setSelected(null); setSessionRecorded(false); setOptionOrders(shuffledOrders(pool)); setSecondsLeft(level === 'FE' ? 90 * 60 : 120 * 60); setBreakSeconds(10 * 60); setView('quiz'); }
  function chooseAnswer(answer: number) { if (!question || revealed) return; setSelected(answer); setAnswers((v) => ({ ...v, [question.id]: answer })); if (answers[question.id]===undefined) recordProgress(question, answer); }
  function goToQuestion(target:number) {setIndex(target);setSelected(answers[partPool[target].id]??null);}
  function goNext() { if (!question) return; if (index < partPool.length - 1) {goToQuestion(index+1);return;} if (level === 'FE' && part === 'A') setView('break'); else setView('result'); }
  ...
  function localizedForUi(item: Question) { return originalText[item.id][system]; }
  ...
  const yearSummary = examYears.map((year) => {
    const sessions=useOfficial
      ? (year===2026?officialStorage.history.filter(a=>a.mode!=='learn').map(a=>{const result=summarize(officialPack,a);return {correct:result.correct,total:result.total,passed:result.passed};}):[])
      : relevantSessions.filter(item=>item.year===year);
    const best=sessions.length?Math.max(...sessions.map(item=>item.total?Math.round(item.correct/item.total*100):0)):null;
    return {year,sessions,best,passed:sessions.some(item=>item.passed)};
  });

  const questionLangCode = system==='ITPEC'?'en':'ja';
  if (officialEntry) return <OfficialExams language={uiLanguage} onLanguage={changeUiLanguage} entry={officialEntry} onExit={()=>{setOfficialEntry(null);refreshOfficial();setView('setup');}} />;
  return <main className="min-h-screen bg-background text-foreground" lang={uiLanguage}>
    ...
    {view === 'setup' && <section className="mx-auto grid max-w-7xl gap-8 px-5 py-8 lg:grid-cols-[1fr_360px] lg:px-8 lg:py-12">
      ...
      <div className="space-y-7">
        ...
        <ChoiceSection number="04" title={label('Вариант','Paper','問題セット')}>
          <div className="grid gap-3 sm:grid-cols-2">
            {officialAvailable&&<ChoiceCard active={bankChoice==='official'} onClick={()=>setBankChoice('official')} title={officialPaperLabel} description={label('Официальный PDF · оригинал','Official PDF · original','公式PDF・原文')}/>}
            <ChoiceCard active={!useOfficial} onClick={()=>setBankChoice('mvp')} title={label('Учебная выборка MVP','MVP practice sample','MVP練習セット')} description={`${pool.length} ${t.available}`}/>
          </div>
          {!officialAvailable&&<p className="mt-3 text-sm text-muted-foreground">{label('Официальный вариант за этот год ещё не перенесён. Доступна только короткая учебная выборка.','This official paper has not been imported yet. Only a short practice sample is available.','この年の公式問題は未収録です。短い練習セットのみ利用できます。')}</p>}
          {useOfficial&&<p className="mt-3 text-sm leading-6 text-muted-foreground">{label('Все 100 вопросов — изображения из оригинального PDF. Проверенных подробных разборов:','All 100 questions use original PDF images. Checked detailed lessons:','100問すべて原本PDFの画像です。確認済みの詳しい解説：')} {officialLessonCount}/100.</p>}
        </ChoiceSection>
        <ChoiceSection number="05" title={t.mode}><div className="grid gap-3 md:grid-cols-3">{(Object.keys(modeInfo) as Mode[]).map((item) => { const Icon = modeInfo[item].icon; return <button key={item} onClick={() => setMode(item)} className={`mode-card ${mode === item ? 'mode-card-active' : ''}`}><Icon className="size-5" /><span className="mt-4 block font-semibold">{modeInfo[item].title}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{modeInfo[item].description}</span></button>; })}</div></ChoiceSection>
      </div></div>
      <aside className="lg:pt-20"><div className="sticky top-28 overflow-hidden rounded-3xl border bg-card shadow-[0_25px_70px_rgba(15,35,42,.09)]"><div className="border-b bg-[var(--ink)] p-6 text-white"><div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-[.18em] text-white/55">{t.session}</span><Sparkles className="size-5 text-[var(--lime)]" /></div><p className="mt-7 text-3xl font-bold">{system} / {level}</p><p className="mt-2 text-sm text-white/65">{examYear} · {modeInfo[mode].title}</p></div><div className="space-y-5 p-6"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 size-5 text-primary" /><div><p className="text-sm font-semibold">{t.verified}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{examFact(level, uiLanguage)}. {breakFact(system, level, uiLanguage)}.</p></div></div><div className="rounded-2xl bg-[var(--mint)] p-4"><p className="text-xs font-semibold text-[var(--mint-ink)]">{t.localProgress}</p><p className="mt-1 text-2xl font-bold">{shownProgress.attempts}</p><p className="text-xs text-muted-foreground">{t.answers} · {shownProgress.attempts ? Math.round(shownProgress.correct / shownProgress.attempts * 100) : 0}% {t.correctShort}</p></div><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.12em] text-muted-foreground"><BarChart3 className="size-4" />{t.examsByYear}</div><div className="space-y-2">{yearSummary.map((item) => <div key={item.year} className="flex items-center justify-between rounded-xl border bg-background px-3 py-2.5"><div><p className="text-sm font-semibold">{item.year}</p><p className="text-[11px] text-muted-foreground">{item.sessions.length ? `${item.sessions.length} ${t.attempts} · ${t.best} ${item.best}%` : t.notTaken}</p></div>{item.passed ? <span className="flex items-center gap-1 text-xs font-semibold text-[var(--success)]"><Trophy className="size-4" />{t.passed}</span> : <span className="text-xs text-muted-foreground">—</span>}</div>)}</div></div><div className="rounded-2xl bg-muted/70 p-4 text-xs leading-5 text-muted-foreground">{examYear}: {useOfficial?100:pool.length} {t.available}.</div><Button className="h-12 w-full rounded-xl text-base" onClick={startQuiz} disabled={!pool.length||(useOfficial&&(!officialLoaded||!!officialStorage.active))}>{t.start} <ArrowRight data-icon="inline-end" /></Button>
        {useOfficial&&officialStorage.active&&<div className="rounded-xl border p-4"><p className="text-sm">{label('Сохранённая попытка','Saved attempt','保存済みの受験')} · {modeInfo[officialStorage.active.mode].title} · {Object.keys(officialStorage.active.answers).length}/100</p><Button className="mt-3 w-full" variant="outline" onClick={()=>setOfficialEntry({attemptId:officialStorage.active!.id})}>{label('Продолжить','Resume','再開')}</Button></div>}
        {useOfficial&&officialStorage.history.length>0&&<details><summary className="cursor-pointer text-sm font-semibold">{label('История этого варианта','Paper history','この問題セットの履歴')}</summary><div className="mt-3 space-y-2">{officialStorage.history.map(a=><button key={a.id} className="w-full rounded-xl border p-3 text-left text-sm" onClick={()=>setOfficialEntry({attemptId:a.id})}><strong>{officialPaperLabel}</strong><span className="block">{modeInfo[a.mode].title} · {summarize(officialPack,a).correct}/100</span><span className="text-xs text-muted-foreground">{new Date(a.finishedAt!).toLocaleString(uiLanguage)}</span></button>)}</div></details>}
        </div></div></aside>
    </section>}
    ...
    {view === 'quiz' && question && <section className="mx-auto max-w-5xl px-5 py-7 lg:px-8 lg:py-10"><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><div className="flex items-center gap-2"><Badge>{level === 'FE' ? `${level} · ${part}` : level}</Badge><Badge variant="outline">{modeInfo[mode].title}</Badge><Badge variant="outline">{t.examLanguage}: {system === 'ITPEC' ? t.english : t.japanese}</Badge></div><p className="mt-3 text-sm text-muted-foreground">{t.question} {index + 1} {t.of} {partPool.length} · {domainLabel(question.domain)} / {topicLabels[question.topic][uiLanguage]}</p></div>{mode === 'exam' && <div className="flex items-center gap-2 rounded-xl border bg-card px-4 py-2.5 font-mono text-lg font-semibold tabular-nums shadow-sm"><Timer className="size-5 text-primary" /> {formatTime(secondsLeft)}</div>}</div><Progress value={((index + (selected !== null ? 1 : 0)) / partPool.length) * 100} className="mb-7 [&_[data-slot=progress-indicator]]:bg-primary" />
      <details className="mb-5 rounded-xl border bg-card p-4"><summary className="cursor-pointer font-semibold">{label('Навигация по вопросам','Question navigation','問題ナビゲーション')}</summary><div className="mt-3 grid grid-cols-5 gap-2 sm:grid-cols-10">{partPool.map((item,i)=><button key={item.id} aria-label={`${t.question} ${i+1}`} aria-current={index===i?'step':undefined} className={`exam-nav ${answers[item.id]!==undefined?'answered':''} ${index===i?'current':''}`} onClick={()=>goToQuestion(i)}>{i+1}{answers[item.id]!==undefined&&<Check className="size-3"/>}</button>)}</div></details>
      <article className="rounded-3xl border bg-card p-5 shadow-[0_22px_60px_rgba(15,35,42,.07)] sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Badge className="bg-[var(--mint)] text-[var(--mint-ink)]">{question.year}</Badge></div><span className="text-right text-[11px] text-muted-foreground">{t.source} · {question.year}</span></div><h2 className="mt-6 text-xl font-semibold leading-8 tracking-[-.015em] sm:text-2xl" lang={questionLangCode}>{questionText?.prompt}</h2>
    ...
    {view === 'result' && <section className="mx-auto min-h-[calc(100vh-74px)] max-w-4xl px-5 py-12"><div className="w-full rounded-3xl border bg-card p-6 shadow-[0_25px_80px_rgba(15,35,42,.09)] sm:p-10"><div className="grid items-center gap-8 sm:grid-cols-[180px_1fr]"><div className="result-ring" style={{ '--score': `${scorePercent}%` } as React.CSSProperties}><div><strong>{correctCount}</strong><span>{t.of} {pool.length}</span></div></div><div><Badge className={sessionPassed ? 'bg-[var(--mint)] text-[var(--mint-ink)]' : ''}>{mode === 'learn' ? t.sessionDone : sessionPassed ? t.examPassed : t.thresholdMissed}</Badge><h2 className="mt-4 text-3xl font-bold tracking-tight">{examYear}: {scorePercent}%</h2><p className="mt-3 leading-7 text-muted-foreground">{mode === 'learn' ? t.learnDone : level === 'IP' ? t.ipRule : `${t.feRule}: A — ${partScore('A')}%, B — ${partScore('B')}%.`}</p></div></div><div className="mt-9 grid gap-3 sm:grid-cols-3">{['Technology','Management','Strategy'].map((domain) => { const items = pool.filter((item) => item.domain === domain || (domain === 'Technology' && ['Algorithm','Security'].includes(item.domain))); const correct = items.filter((item) => answers[item.id] === item.answer).length; return <div key={domain} className="rounded-2xl bg-muted/65 p-4"><p className="text-xs text-muted-foreground">{domainLabel(domain)}</p><p className="mt-2 text-2xl font-bold">{correct} / {items.length}</p></div>; })}</div>
