...
import { lessons, topicLabels, uiCopy, type UiLanguage } from './kiso-i18n';
...
type QuestionLanguage = 'original' | 'ru' | 'interface';
...
export default function Home() {
  ...
  const questionText = question ? (questionLanguage === 'original' ? originalText[question.id][system] : questionLanguage === 'ru' || uiLanguage === 'ru' ? { prompt: question.prompt, options: question.options } : originalText[question.id][interfaceQuestionSystem]) : null;
  ...
  const questionLangCode = questionLanguage === 'original' ? (system === 'ITPEC' ? 'en' : 'ja') : questionLanguage === 'ru' ? 'ru' : uiLanguage;
...
function LessonDetails({ lesson, copy, open = false }: { lesson: { core: string; method: string; example: string; pitfall: string }; copy: (typeof uiCopy)[UiLanguage]; open?: boolean }) { return <details className="lesson-details" open={open}><summary><BookOpenCheck className="size-4" />{copy.learnTopic}</summary><div className="lesson-grid"><section><strong>{copy.principle}</strong><p>{lesson.core}</p></section><section><strong>{copy.method}</strong><p>{lesson.method}</p></section><section><strong>{copy.example}</strong><p>{lesson.example}</p></section><section><strong>{copy.pitfall}</strong><p>{lesson.pitfall}</p></section></div></details>; }
