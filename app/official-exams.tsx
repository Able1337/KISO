...
function KeyNotice({pack,question,language}:{pack:ExamPack;question:ExamQuestion;language:UiLanguage}) {
 const disputed=(pack.id==='itpec-ip-2025-autumn'&&question.number===42)||(pack.id==='itpec-fe-2025-spring'&&question.part==='A'&&question.number===6);
 if(!disputed)return null;
 return <p role="note" className="mt-3 rounded-xl border border-amber-400 bg-amber-50 p-3 text-sm text-amber-950">{{ru:'Замечание к официальному ключу: обнаружено противоречие с содержанием задания. Для учебного подсчёта сохранён опубликованный ключ; подробности — в разборе.',en:'Official-key caveat: the published answer conflicts with the question content. Practice scoring preserves the official key; see the explanation for details.',ja:'公式正解への注記：問題内容との矛盾があります。学習採点は公開キーを保持し、詳細を解説に示します。'}[language]}</p>;
}
...
export default function OfficialExams({pack, language, onLanguage, onExit, entry}: {pack:ExamPack; language:UiLanguage; onLanguage:(lang:UiLanguage)=>void; onExit:()=>void; entry:OfficialEntry}) {
  ...
  const paper=`${pack.system} ${pack.level} · ${pack.system==='IPA'?pack.id==='ipa-fe-2022-sample'?label('демонстрационный комплект 2022','demonstration set 2022','2022年サンプル問題セット'):label(`публикация ${pack.year}`,`published ${pack.year}`,`${pack.year}年度公開問題`):pack.season==='autumn'?label(`октябрь ${pack.year}`,`October ${pack.year}`,`${pack.year}年10月`):label(`апрель ${pack.year}`,`April ${pack.year}`,`${pack.year}年4月`)}`;
  const lessonFor=(question:ExamQuestion)=>pack.system==='IPA'?ipaLesson(pack.level,question.part,question.number,language,pack.id):pack.id==='itpec-ip-2026-spring'?officialLesson(question.number,language):pack.id==='itpec-fe-2026-spring'?feLesson(question.part,question.number,language):itpecArchiveLesson(pack.id,question.part,question.number,language);
    ...
    <section className="mx-auto max-w-6xl px-5 py-8">
      ...
      {attempt&&attempt.status==='active'&&attempt.stage!=='break'&&q&&<>
        ...
        <article className="mt-5 rounded-3xl border bg-card p-4 sm:p-7">
          ...
          <div className="mt-6 space-y-3">{attempt.orders[q.id].map((id,i)=>{const option=q.options.find(o=>o.id===id)!;return <button key={id} disabled={revealed} className={`official-option ${revealed&&id===q.answerId?'review-answer-correct':revealed&&attempt.answers[q.id]===id?'review-answer-wrong':attempt.answers[q.id]===id?'selected':''}`} aria-pressed={attempt.answers[q.id]===id} aria-label={`${t.option} ${String.fromCharCode(65+i)}: ${option.text||t.diagram}`} onClick={()=>update(answerQuestion(pack,attempt,q.id,id,Date.now()))}><span className="answer-letter">{String.fromCharCode(65+i)}</span><img src={option.image.src} width={option.image.width} height={option.image.height} alt={option.text||t.diagram} lang={pack.originalLanguage}/>{attempt.answers[q.id]===id&&(revealed&&id!==q.answerId?<XCircle className="size-5 shrink-0 text-destructive"/>:<Check className="size-5 shrink-0 text-primary"/>)}</button>;})}</div>
          {revealed&&<div data-testid="explanation" className={`mt-6 rounded-2xl border p-5 ${attempt.answers[q.id]===q.answerId?'explanation-correct':'explanation-wrong'}`}>
            ...
            <KeyNotice pack={pack} question={q} language={language}/>{lesson?<><p className="mt-3 leading-7">{lesson.core}</p><a className="search-topic" href={`https://www.google.com/search?q=${encodeURIComponent(lesson.search)}`} target="_blank" rel="noreferrer"><Search className="size-4"/><span><strong>{copy.searchFor}</strong>{lesson.search}</span></a><div className="study-next"><Lightbulb className="size-4"/><span><strong>{copy.studyNext}</strong>{lesson.next}</span></div><details className="lesson-details" key={q.id}><summary>{copy.learnTopic}</summary><div className="lesson-body"><p>{lesson.detail}</p><LessonSources lesson={lesson}/></div></details></>:<p className="mt-3 text-muted-foreground">{t.pendingLesson}</p>}
          </div>}
        ...
        <div className="mt-5 flex flex-wrap justify-between gap-3"><Button variant="ghost" onClick={onExit}>{t.leave}</Button><a className="inline-flex items-center gap-2 text-sm text-primary underline" href={`${questionPdf(q)}#page=${pack.referencePage??(pack.parts?2:pack.system==='IPA'?50:pack.id==='itpec-ip-2026-spring'?39:1)}`} target="_blank" rel="noreferrer">{t.reference}<ExternalLink className="size-4"/></a></div>
      </>}
