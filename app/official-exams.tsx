...
function PromptImages({question}:{question:ExamQuestion}) {
  return <div className="space-y-5">{(question.promptPages??[question.prompt]).map((asset,i)=><img key={asset.src} className="exam-prompt" loading={i?'lazy':undefined} src={asset.src} width={asset.width} height={asset.height} alt={i?`${question.source} · ${i+1}`:question.promptText} lang="en"/>)}</div>;
}
  ...
  return <main lang={language} className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-6xl px-5 py-8">
      ...
      {attempt&&attempt.status==='active'&&attempt.stage!=='break'&&q&&<>
        <div className="flex flex-wrap items-center justify-between gap-3"><div><Badge>{paper}</Badge> <Badge variant="outline">{attempt.mode==='learn'?t.learn:attempt.mode==='mock'?t.mock:t.exam}</Badge><p className="mt-2 text-sm text-primary">{t.language}</p></div>{remaining!==null&&<div className="rounded-xl border bg-card px-4 py-3 font-mono text-xl" aria-label={t.exam}><Clock3 className="mr-2 inline size-5"/>{timeLabel(remaining)}</div>}</div>
        ...
        <article className="mt-5 rounded-3xl border bg-card p-4 sm:p-7">
          ...
          <div className="exam-image-scroll mt-5"><PromptImages question={q}/></div>
        ...
        <details className="mt-5 rounded-xl border p-4"><summary className="cursor-pointer text-sm font-semibold">{t.source}</summary><p className="mt-3 text-sm leading-6 text-muted-foreground">{pack.parts?label('Оригинальные многостраничные задания FE A/B. Заголовки таблиц повторены у каждого ответа; порядок ответов перемешивается. Нумерация A и B независимая.','Original multi-page FE A/B questions. Table headers repeat for each shuffled answer. A and B have separate numbering.','FE A/Bの原文を複数ページのまま収録。各選択肢に表見出しを繰り返し、順序を変更します。AとBの番号は独立です。'):t.notes}</p><a className="mt-3 block text-primary underline" href={pack.sourceUrl} target="_blank" rel="noreferrer">ITPEC.org ↗</a></details>
        <div className="mt-5 flex flex-wrap justify-between gap-3"><Button variant="ghost" onClick={onExit}>{t.leave}</Button><a className="inline-flex items-center gap-2 text-sm text-primary underline" href={`${questionPdf(q)}#page=${pack.parts?2:39}`} target="_blank" rel="noreferrer">{t.reference}<ExternalLink className="size-4"/></a></div>
      </>}
      {attempt?.status==='active'&&attempt.stage==='break'&&<div className="mx-auto mt-10 max-w-2xl rounded-3xl border bg-card p-8 text-center"><Badge>{paper} · A → B</Badge><h1 className="mt-5 text-3xl font-bold">{copy.partDone}</h1><p className="mt-4 leading-7">{copy.breakText}</p><p className="mt-3 text-muted-foreground">{label('Учебная пауза без ограничения времени. Вторая часть: 20 вопросов; в режиме экзамена — 100 минут.','Untimed practice break. Subject B: 20 questions; 100 minutes in timed mode.','練習用の休憩に時間制限はありません。Bは20問、試験モードでは100分です。')}</p><Button className="mt-6" onClick={()=>{const current=Date.now();setNow(current);update(startNextPart(pack,attempt,current));}}>{copy.startB}</Button><Button className="mt-6 ml-3" variant="outline" onClick={onExit}>{t.leave}</Button></div>}
      {attempt?.status==='finished'&&result&&<>
        ...
        <h2 className="mt-9 text-2xl font-semibold">{t.review}</h2><p className="mt-2 text-sm text-muted-foreground">{pack.parts?label('Все 80 вопросов, официальные ответы и подробные разборы доступны.','All 80 questions, official answers and detailed lessons are available.','全80問、公式正解、詳しい解説を収録しています。'):t.pending} ({lessonCount}/{total})</p>
