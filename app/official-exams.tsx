...
export default function OfficialExams({pack, language, onLanguage, onExit, entry}: {pack:ExamPack; language:UiLanguage; onLanguage:(lang:UiLanguage)=>void; onExit:()=>void; entry:OfficialEntry}) {
  ...
  const paper=`${pack.system} ${pack.level} · ${pack.system==='IPA'?label('публикация 2026','published 2026','2026年度公開問題'):label('апрель 2026','April 2026','2026年4月')}`;
  const lessonFor=(question:ExamQuestion)=>pack.system==='IPA'?ipaLesson(pack.level,question.part,question.number,language):pack.id==='itpec-ip-2026-spring'?officialLesson(question.number,language):pack.id==='itpec-fe-2026-spring'?feLesson(question.part,question.number,language):null;
