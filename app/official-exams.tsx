  ...
  return <main lang={language} className="min-h-screen bg-background text-foreground">
    ...
    <section className="mx-auto max-w-6xl px-5 py-8">
      ...
      {attempt?.status==='finished'&&result&&<>
        ...
        <h2 className="mt-9 text-2xl font-semibold">{t.review}</h2><p className="mt-2 text-sm text-muted-foreground">{pack.parts?label('Все вопросы и ключи перенесены. Подробные разборы готовы для A1–A40; остальные ещё готовятся.','All questions and keys are imported. Detailed lessons cover A1–A40; the rest are in preparation.','全問題と正解を収録。A1～A40の詳しい解説があり、残りは準備中です。'):t.pending} ({lessonCount}/{total})</p>
